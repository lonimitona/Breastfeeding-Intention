---
title: "Assignment A2 - Annotated Script"
author: '2149508'
date: '2022-07-11'
output:
...
###  Let's access the csv files and explore the data
```{r}
wales_data <- read.csv('Born in Wales Data June 2022.csv')
head(wales_data)
```
#Here we can see that the default for most of the variables is that they are character variables although a majority contain categorical data and some contain date-time data. The column names are too long and need to be adjusted. We don't need the last column which asks the participants to submit the questionnaire, so we will exclude that. We will also not be needing the WIMD columns.


### Let's exclude the less useful columns
```{r}
...
### Let's rename the columns to shorter names which are easier to read and understand
```{r}
...
### Let's view the adjusted column names
```{r}
head(wales_data_2022)
```


### Now let's change all the columns that contain categorical data from character to factor variables.
```{r}
...
### Let's see the adjusted variables
```{r}
str(wales_data_2022)
```
#Here we see that we can now easily access the different levels of the categorical variables. But some variables are showing up to 14 levels which is a lot. There might be duplicate levels. 


### Let's see all the levels of the categorical variables
```{r}
lapply(wales_data_2022, levels)
```

### Let's group the levels
```{r}
...
```
#After grouping the levels/categories, we can see that there are empty cells in almost all the variables. This is because some questions were not relevant to some observations and in some cases the paticiant didn't answer those questions, so we have many blank spaces. But there are some observations where none of the questions were answered and this will not be useful for our analysis. So let's figure out a way to deal with them.


### Let's check for the number of empty cells in each column
```{r}
colSums(is.na(wales_data_2022))
```
#Some columns are returning no empty cells when they actually have multiple empty cells. It seems R is not recognizing the blank cells as empty. So let's fill all the blank cells with NA
...
### Set threshold to remove rows that have NAs that are more than 60
```{r}
...
### Now let's see the empty cells in each column
```{r}
colSums(is.na(wales_data_2022))
```


### Recode and aggregate set of questions for Low mood (Depression), Anxiety, Stress and Physical Activity
```{r}
...
### Some categories were not grouped, let's tidy those up
```{r}
...
### Let's change the variables that contain date data from characters to date-time variables
```{r}
...
### Weight is in metres, let's convert it to cm and calculate the BMI for each observation
```{r}
wales_data_2022 <- wales_data_2022 %>% 
...
```
### All the blank spaces have been filled with NAs. But we also note that because we grouped the levels, there are now redundant levels. Let's take care of them.
```{r}
wales_data_2022 <- droplevels(wales_data_2022)
```


### Let's group the occupations
```{r}
...
### Create a variable to group Household number, Number of Children, BMI, number of people contacted, Depression, Anxiety, Physical activity
```{r}
wales_data_2022 <- wales_data_2022 %>% 
...
### Let's access the follow up data
```{r}
...
### The dates are in character format, let's convert them to date-time format
```{r}
...
### Create variable for those who initiated breastfeeding
```{r}
...
### Create variable for breastfeeding duration
```{r}
...
```
The probability of a woman breastfeeding up to a year is 30%

### Merge followup data to the main dataset
```{r}
...
#### Length of work hours and breastfeeding duration
```{r}
surv1 <- survfit(Surv(duration_bf, censor) ~ Length_work_hrs, data = wales_data_2022)

print(surv1)

ggsurvplot(surv1, data=wales_data_2022, 
           legend.labs=c('Full time','Part-time', 'Other'))
...
### Marital status and breastfeeding duration
```{r}
#Cox regression
...
### Let's draw the demographic table to summarize the characteristics 
```{r}
...
### Create Logistic Regression table to summarize the associations
```{r}
...
mod3 <- glm(Intent_BF ~ is_event_covid_related + emotional_finance_support_at_this_time + children + Smoking + Alcohol, 
            data = model, family = binomial)
...
### Let's train the data and draw decision tree
```{r}
...
```
#Here we can see the results of our confusion matrix but the positive class is not right. Let's solve this.


### See levels of Breastfeeding intent
```{r}
levels(dt_Train$Intent_BF)
```
#No intent comes first, that why it was selected as the positive class by the confusion matrix, so we will relevel the factor
